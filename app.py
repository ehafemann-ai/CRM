        ...
        with st.form("login_form"):
            ...
            if st.form_submit_button("Iniciar Sesión", use_container_width=True):
                ...
                if user_data and user_data.get('pass') == password:
                    st.session_state['auth_status'] = True; st.session_state['current_user'] = username; st.session_state['current_role'] = user_data.get('role', 'Comercial')
...
# ==============================================================================
# 2. CONEXIÓN SEGURA A DATOS (GITHUB PRIVADO)
# ==============================================================================
...
def cargar_datos_seguros():
    """Descarga el Excel desde un Repo Privado usando Token"""
    try:
        ...
        if response.status_code == 200:
            # Cargar Excel desde la memoria (bytes)
            ...
            st.error(f"Error de conexión al repositorio privado: {response.status_code}")
            return None, None, None, None, None, None, None
            
    except Exception as e:
        # Fallback para desarrollo local (si tienes el archivo ahí)
        ...
        st.error(f"No se pudo cargar la base de precios. Configura los Secrets o sube el archivo. Error: {e}")
        return None, None, None, None, None, None, None

data = cargar_datos_seguros()
if data[0] is None: st.stop()
...
def modulo_dashboard():
    ...
    if not df_s.empty: c2.plotly_chart(px.bar(df_s.groupby(['pais', 'moneda'])['total'].sum().reset_index(), x='pais', y='total', color='moneda'), use_container_width=True)
