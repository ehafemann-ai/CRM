...
def modulo_seguimiento():
    ...
    # --- FILTRO POR EQUIPO/PROPIO ---
    if curr_role == 'Comercial':
        my_team = st.session_state['users_db'][curr_user].get('equipo', 'N/A')
        # Lista de nombres de mi equipo
        team_names = [u['name'] for k, u in st.session_state['users_db'].items() if u.get('equipo') == my_team]
        # Filtrar DF: Solo veo lo mío o lo de mi equipo
        df = df[df['vendedor'].isin(team_names)]
    
    # --- FILTRO ABIERTAS/HISTORIAL ---
    c1, c2 = st.columns([3, 1])
    with c1:
        st.info("ℹ️ Gestión: Cambia estado a 'Aprobada' para que Finanzas facture.")
    with c2:
        ver_historial = st.checkbox("📂 Ver Historial Completo", value=False)
    
    if not ver_historial:
        # Por defecto solo muestra las "vivas"
...
def modulo_finanzas():
    ...
    with tab_billing:
        ...
        if to_bill.empty: st.success("¡Excelente! No hay cotizaciones pendientes de facturar.")
        else:
            for i, r in to_bill.iterrows():
                with st.container():
                    ...
                    else: st.warning("⚠️ Vista de PDF no disponible (cotización antigua).")
            ...
            if st.form_submit_button("Crear Usuario"):
                if not new_email or not new_pass: st.error("Correo y contraseña obligatorios")
                    ...
                    if github_push_json('url_usuarios', users, st.session_state.get('users_sha')):
                        ...
                        st.success(f"Usuario {new_email} creado"); time.sleep(1); st.rerun()
